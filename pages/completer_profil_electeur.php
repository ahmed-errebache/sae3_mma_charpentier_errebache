...
if (isset($_SESSION['profil_complet']) && $_SESSION['profil_complet'] === true) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($date_naissance) || empty($sexe) || empty($nationalite)) {
        $error = 'Tous les champs sont obligatoires.';
    } else {
        $age = calculerAge($date_naissance);
...
<main class="flex-1">
    <div class="min-h-[70vh] flex items-center justify-center px-4 sm:px-6 lg:px-8 py-12">
        <div class="w-full max-w-md bg-white rounded-xl shadow-lg p-6 sm:p-8">
            <h1 class="text-2xl font-bold text-center text-noir mb-2">Completez votre profil</h1>
            <p class="text-sm text-gray-600 text-center mb-6">
                Pour finaliser votre inscription, veuillez completer ces informations
            </p>
            ...
            <form action="completer_profil_electeur.php" method="post" class="space-y-6">
                <div>
